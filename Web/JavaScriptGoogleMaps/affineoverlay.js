...
/**
 * Adds the image in the top left of the current map viewport. The overlay can
 * be transformed via three control points, and translated via a larger control
 * point that sits in the middle of the image overlay.
 */
overlaytiler.AffineOverlay.prototype.onAdd = function() {
	...
	for (var i = 0, dot; dot = dots[i]; ++i) {
		...
		google.maps.event.addListener(dot, 'dragend', this.setMapDraggable_
				.bind(this, true));
	...
	google.maps.event.addListener(mover, 'dragstart', this.setMapDraggable_
			.bind(this, false));

	google.maps.event.addListener(mover, 'dragend', this.setMapDraggable_.bind(
			this, true));
...
/**
 * Notify that the canvas should be rendered. Essentially limits rendering to a
 * max of 66fps.
 * 
 * @private
 */
overlaytiler.AffineOverlay.prototype.renderCanvas_ = function() {
	if (this.renderTimeout)
		return;
	this.renderTimeout = window.setTimeout(this.forceRenderCanvas_.bind(this),
			15);
};
 ...
 * @private
 * @param {boolean}
 *            draggable Whether the map should be draggable.
 */
...
overlaytiler.AffineOverlay.prototype.getVirtualDot_ = function() {
	var dots = this.dots_;
	return new google.maps.Point(dots[0].x + dots[2].x - dots[1].x, dots[0].y
			+ dots[2].y - dots[1].y);
};
...
overlaytiler.AffineOverlay.prototype.getTopLeftPoint_ = function() {
	...
	return new google.maps.Point(Math.min(dots[0].x, dots[1].x, dots[2].x,
			virtualDot.x), Math.min(dots[0].y, dots[1].y, dots[2].y,
			virtualDot.y));
};
...
overlaytiler.AffineOverlay.prototype.getDotLatLngs = function() {
	...
	var dots = this.dots_;
	for (var i = 0, dot; dot = dots[i]; ++i) {
		result.push(proj.fromDivPixelToLatLng(new google.maps.Point(dot.x,
				dot.y)));
	}
	return result;
};
...
overlaytiler.AffineOverlay.prototype.getVirtualDotLngLat = function() {
	...
	var dots = this.dots_;
	var point = new google.maps.Point(dots[0].x + dots[2].x - dots[1].x,
